...
export default function ManajemenAkun() {
  ...
  const handleSaveEdit = async (e) => {
    ...
    try {
      ...
      await showAlert({ type: "success", title: "Data Diperbarui!", message: "Data pengguna berhasil disimpan." });
      setIsEditModalOpen(false);
      fetchUsers();
    } catch (error) {
      await showAlert({ type: "error", title: "Gagal!", message: "Gagal memperbarui data. Coba lagi." });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id, nama) => {
    const confirmed = await showAlert({
      type: "confirm",
      title: "Hapus Akun?",
      message: `Akun "${nama}" akan dihapus permanen dari sistem. Tindakan ini tidak bisa dibatalkan.`,
    });
    if (!confirmed) return;

    try {
      await axios.delete(`${API_BASE_URL}/api/users/${id}`);
      await showAlert({ type: "success", title: "Akun Dihapus!", message: `Akun ${nama} berhasil dihapus dari sistem.` });
      fetchUsers();
    } catch (error) {
      console.error("Gagal menghapus:", error);
      await showAlert({ type: "error", title: "Gagal!", message: "Gagal menghapus akun. Pastikan server backend menyala." });
    }
  };
