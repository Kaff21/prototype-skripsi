...
export default function AnggotaAktif() {
  ...
  const hapusAnggota = async (id, nama) => {
    if (window.confirm(`Apakah Anda yakin ingin mengeluarkan ${nama} dari anggota?`)) {
      try {
        // Karena ini anggota aktif, kita bisa ubah statusnya kembali jadi pending atau hapus.
        // Di sini kita ubah statusnya menjadi 'pending' (atau ditolak jika ada endpointnya).
        // Kita gunakan endpoint status yang sudah ada:
        await axios.patch(`${API_BASE_URL}/api/users/${id}/status`, { status: 'pending' });
        setAnggotaList(prev => prev.filter(a => a.id !== id));
        alert(`${nama} berhasil dikeluarkan.`);
      } catch (error) {
